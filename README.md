...
Just open the mock prototype in a browser — it runs on procedural data:

```bash
open examples/webapp/prototype.html        # macOS  (or double-click the file)
```

### Run the web app (live, with Earth Engine)

```bash
# 1. environment
python -m venv rsembed
rsembed/bin/pip install -e .                                   # core rs-embed
rsembed/bin/pip install fastapi "uvicorn[standard]" python-multipart   # web-app deps

# 2. authenticate Earth Engine once + set a Cloud project
rsembed/bin/earthengine authenticate
rsembed/bin/earthengine set_project YOUR_GEE_PROJECT

# 3. (optional) build cached assets + the pretrained head used by the "Use" tab
EARTHENGINE_PROJECT=YOUR_GEE_PROJECT rsembed/bin/python examples/build_demo_cache.py --only corn_heads

# 4. launch the server
EARTHENGINE_PROJECT=YOUR_GEE_PROJECT \
  rsembed/bin/python -m uvicorn server:app --app-dir examples/webapp --port 8000
```
...
| `clay`            | S2 L2A 10-band (+ latlon/time/gsd/wavelengths) | 10m   | [model release v1.5, 2024](https://clay-foundation.github.io/model/)            | [link](https://github.com/Clay-foundation/model)             |
