...
- **Rectangular ROIs on square-input models are fetched as enlarged squares of real imagery and cropped back to the requested region (fetch-square).** Instead of synthetically padding a rectangle to square, the fetch enlarges the short side with real spatial context and the output grid / pooled vector is restricted to the ROI window afterwards — consistently across `get_embedding` and every `export_batch` path (single, batch, tiled). Under `tile` input_prep the rectangle is fetched directly (tiles are cut square from any-aspect imagery, so no enlargement is needed). See `docs/spatial_roi.md`.
