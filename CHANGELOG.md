...
- **Combined-export manifest dropped all but one model.** In a combined `export_batch` over multiple models, the embedding arrays for every model were written to the `.npz` correctly, but the manifest JSON's `models` list retained only one entry (order/run-dependent), so `rs_embed.load_export()` silently reported a single model. The cause was a desync in `pipelines/exporter.py::_run_combined`: the `_write_ckpt` closure captured the outer-scope `manifest`, while `pipelines/combined_flow.py::run_pending_models` accumulated each model's entry into its *own* local `manifest` (reassigned each iteration to the fresh dict returned by `combined_write_checkpoint`). After the first checkpoint write the two diverged, so every subsequent checkpoint re-wrote the stale single-model manifest. `run_pending_models` now passes its current accumulated `manifest` explicitly into `write_checkpoint_fn`, and `_write_ckpt` writes that manifest, so the persisted `models` list always matches the `embeddings__*` arrays in the npz. Added `test_export_batch_combined_manifest_lists_all_models` as a regression test.
