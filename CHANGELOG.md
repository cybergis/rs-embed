...
- **Per-zone embeddings** — `embed_zones(model, zones=..., zone_id_field=..., temporal=...)` returns one vector per POLYGON, for analysis that works in administrative or management units (census tract, county, field, watershed) rather than bounding boxes. `zones` takes a path in any format geopandas reads, a `GeoDataFrame`, or an iterable of `(id, geometry)`; the extent is swept in tiles, each tile embedded as a grid, and statistics accumulated for the pixels inside each polygon — no full cube is assembled, so 801 Chicago tracts (25.8 M pixels x 64 bands ~ 6.6 GB) cost one vector per zone in memory. `ZoneEmbeddings.to_frame()` gives the `zone_id, pixels, area_km2, e000…` table a downstream model consumes, and `.rollup({zone_id: parent})` combines zones into coarser units exactly. Each zone keeps `total` (per-band sum) and `pixels` alongside `mean`, because a mean cannot be re-aggregated — averaging the means of unequal zones is not the mean of their union — and `pixels` is the honest measure of a zone's support, which is what says whether a model fitted on small zones is extrapolating on a large one. Tiles are requested on a grid snapped to whole multiples of `scale_m` in **EPSG:3857** and the affine is derived from the requested bounds and the returned shape, then checked: `scale_m` is Web Mercator metres, running `1/cos(latitude)` longer than ground metres (a 0.02x0.01 degree bbox at 40.07N returns 224x146, not the 170x111 a ground-metre reading predicts), so `meta["pixel_ground_m"]` reports what a pixel actually covers and any pixel-size disagreement over 2% lands in `meta["pixel_size_warnings"]` instead of silently shifting every zone boundary. A `zone_id_field` naming a column that does not exist raises with the columns that do, rather than falling back to row numbers and producing vectors that never join. Needs geopandas and rasterio: `pip install "rs-embed[zones]"`. See [docs/api_zones.md](docs/api_zones.md).
...
- **Demo cache builder, and the two prediction heads it produces** — `examples/build_demo_cache.py` reproducibly builds the cache behind the I-GUIDE interactive demo, and with it the first downstream heads that consume embeddings on this branch. The web app has served `/api/heads`, `/api/predict`, `/api/predict_example` and `/api/predict_package` all along, but the builder was not here — it lived only on `webapp-viz` — so `/api/heads` always answered `{"task": null, "models": []}` with a note naming a script the branch did not contain, and `/api/predict` returned 404 before it touched an embedding. Two of the seven `--only` targets are heads, each fitting one frozen scikit-learn estimator per embedding model and pickling it as `{task}__{model}.pkl` beside a `heads_meta.json`: `--only heads` is a maize-yield `RandomForestRegressor` over Illinois with labels from the SPAM2020 rasters, reporting a held-out R² per model; `--only corn_heads` is a corn-presence `RandomForestClassifier` with labels sampled from USDA CDL via `stratifiedSample`, which needs **no external label files** and is the path that runs out of the box. Nothing in the server changes to serve either: `_load_head` takes the task from `heads_meta.json` rather than hardcoding one, and `_predict_one` dispatches on `hasattr(reg, "predict_proba")`, returning `P(class 1)` with a `label_pred` for classification and a range check for regression. The remaining targets (`twins`, `timemachine`, `landcover`, `showdown`, `maize`) build the demo's other panels. Costs to know before running: the builder needs a one-time `earthengine authenticate` and network — it is meant to be run by a maintainer ahead of a demo, not by a booth visitor — `--only corn_heads` embeds 120 points across the full model list against Earth Engine (`--quick` reduces that to 30 points and three models), and `--only heads` additionally needs two SPAM2020 CSVs fetched by hand (`spam2020V2r0_global_H_TA.csv`, `spam2020V2r0_global_Y_TA.csv`), so it will not run unattended. The built cache is gitignored: embeddings plus pickled heads run to hundreds of MB. `examples/webapp/` is deliberately not brought over — this branch ignores it, and the deployed `server.py` is a patched copy carrying `/api/zones`; only the builder was missing.
...
- **`examples/play_with_embedding.ipynb`** — a self-contained downstream-task walkthrough over exported embeddings (PCA, clustering, and a small torch prediction head); depends only on public libraries.
