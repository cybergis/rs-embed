...
- **`export_batch` returned square outputs for rectangular ROIs on square-input models** (e.g. an `olmoearth` `grid` covering the whole enlarged square instead of the requested `BBox`). The prefetch square-fetched the ROI but the batch paths dropped the `roi_window_geo` crop-back window that `get_embedding` honors: every `get_embeddings_batch_from_inputs` now accepts the threaded `fetch_metas` and crops grids / ROI-pools vectors per item, the tiled batch path crops the stitched grid once (stripping the image-level window from per-tile metas), and under `tile` input_prep the prefetch now fetches the rectangular ROI directly (`square_input=False`, matching `get_embedding` — no extra imagery). Outputs from `export_batch` now match `get_embedding` for rectangular ROIs on all paths.
