...
- **`tessera` downloads work again: geotessera floor raised to 0.10.** The GeoTessera tile host hard-coded in geotessera < 0.10 (`dl2.geotessera.org`) was retired upstream and now answers HTTP 410 for every tile, so the locked 0.7.5 could not fetch anything. The data moved to Source Cooperative in geotessera 0.10; the tile API `rs-embed` uses (`load_blocks_for_region`, `fetch_embeddings`) is unchanged, so `pyproject.toml` now requires `geotessera>=0.10` and the lock is updated. Note that geotessera writes downloaded tiles to the current working directory by default (`embeddings_dir`); `cache_dir` only covers its registry files.

- **One output grid for every model: Tessera now lands on the common EPSG:3857 grid.** Provider-backed models have always been sampled on EPSG:3857 at the request's `scale_m`, while `tessera` returned its tiles in their native UTM CRS, so grid outputs for the same ROI could not be compared pixel-for-pixel and a ROI crossing a UTM zone boundary was refused (`Tiles have different CRS; cannot mosaic`). Tessera tiles are now resampled (nearest neighbour, each tile through its own CRS) onto the common grid at 10 m — the ROI's EPSG:3857 rectangle snapped outward to the lattice anchored at the projection origin, the same lattice `ee.Projection("EPSG:3857").atScale(...)` samples on — so a tessera grid and a provider-fetched patch for one ROI share pixel edges. A ROI straddling a UTM zone boundary is served with a `UserWarning` about the resampling seam instead of an error. Meta keys changed: `output_crs` is now `EPSG:3857`, `projection_mode="common_grid"`, plus `scale_m`, `transform` (north-up affine), `grid_hw`, `tile_crs` (list of source UTM CRSs), `resampling`, `coverage`; the tile-native `mosaic_hw`/`crop_hw`/`crop_px_window`/`global_transform`/`projection_note` keys and the one-time "product-native tile CRS" warning are gone. The grid definition, resampling and UTM-zone checks live in the new `tools/projection.py`; the Web Mercator formulas previously duplicated in `tools/spatial.py` and `providers/gee_utils.py` (and the GEE request rectangle in `providers/gee.py`) now come from there, so the fetch-square, the GEE region and the common grid cannot drift apart.
...
- **Georeferenced user data: `UserData(crs=..., transform=...)` and `UserData.from_raster(path, ...)`.** A declaration can now carry the CRS and pixel→CRS affine of its array; the footprint is derived from them (so `spatial` may be omitted, and georef-conditioned models such as clay/prithvi accept the declaration), and the array is resampled onto the common EPSG:3857 grid at the model's `scale_m` before embedding — the same grid a provider fetch would have produced, so user rasters and fetched patches reach a model in the same frame. `UserData.from_raster` reads pixels, CRS, transform and pixel size from a GeoTIFF via `rasterio` (optional dependency), detecting the projection instead of asking for hand-computed lon/lat bounds. A UTM raster whose footprint spills into a neighbouring zone warns. `meta["user_input"]["projection"]` records the alignment (`crs`, `output_crs`, `grid_hw`, `transform`, `resampling`, ...); ungeoreferenced declarations pass through unchanged (`{"crs": None, "aligned": False}`). `crs` and `transform` must be given together (`SpecError` otherwise).
...
- **[docs/benchmarks.md](docs/benchmarks.md)** — measured end-to-end latency and throughput of every registered backend (H100 + GEE, 20-point Midwest workload), grouped into single-frame / multi-frame / precomputed, with per-request fetch-vs-compute decomposition and reading guidance. Nav: Models → Latency & Throughput.
