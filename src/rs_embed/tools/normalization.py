...
from .model_defaults import _probe_model_desc

# Backward-compat alias (re-exported by api.py)
_probe_model_describe = _probe_model_desc
...
def _resolve_embedding_api_backend(model_n: str, backend_n: str) -> str:
    ...
    provider_ok = "provider" in allowed or "gee" in allowed

    # Direct match
    if backend_n in allowed:
        if backend_n == "auto" and provider_ok:
            return _default_provider_backend_for_api()
        return backend_n
    if backend_n == "local" and "auto" in allowed and not provider_ok:
        return "auto"
    if has_provider(backend_n) and provider_ok:
        return backend_n

    # Fallback for gee/auto requests
    if backend_n in {"gee", "auto"}:
        for candidate in ("auto", "local"):
