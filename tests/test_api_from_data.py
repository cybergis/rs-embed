...
class _MockFromDataEmbedder(EmbedderBase):
    """Captures the input_chw it receives; no I/O."""
    ...
    def get_embedding(
        ...
        *,
        ...
        return Embedding(
            ...
            meta={"model": self.model_name, "output": output.mode},
        )
...
def _twelve_band_userdata(*, fill_by_channel=True, hw=(8, 8)):
    ...
    return UserData(data=arr, collection="s2", bands=bands)
...
def test_missing_band_refuses_with_band_name():
    ...
    with pytest.raises(ModelError, match=r"lacks \['B4'\]"):
        get_embedding_from_data("mock_from_data", data=data, spatial=_POINT)
...
def test_invalid_userdata_raises_specerror():
    ...
    with pytest.raises(SpecError, match="channels"):
        get_embedding_from_data("mock_from_data", data=bad, spatial=_POINT)
...
def test_grid_output_mode_is_passed_through():
    emb = get_embedding_from_data(
        "mock_from_data",
        data=_twelve_band_userdata(),
        spatial=_POINT,
...
def test_batch_returns_one_embedding_per_item():
    ...
    embs = get_embeddings_batch_from_data(
        "mock_from_data",
        datas=datas,
        spatials=[_POINT, _POINT],
    )
    assert len(embs) == 2
    assert all(e.meta["user_input"]["channel_indices"] == [3, 2, 1] for e in embs)


def test_batch_length_mismatch_refuses():
    with pytest.raises(ModelError, match="length mismatch"):
        get_embeddings_batch_from_data(
            "mock_from_data",
            datas=[_twelve_band_userdata()],
            spatials=[_POINT, _POINT],
        )
...
def test_batch_empty_refuses():
    with pytest.raises(ModelError, match="non-empty"):
        get_embeddings_batch_from_data("mock_from_data", datas=[], spatials=[])
